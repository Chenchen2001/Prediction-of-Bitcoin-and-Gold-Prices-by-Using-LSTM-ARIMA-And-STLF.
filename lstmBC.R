...
library(ggthemes)
library(lubridate)
# draw the data to have a brief look
dataframe <- read.csv('./BITCOIN.csv')

plot(dataframe$Value, type="l", ylab = "Bitcoin Value", xlab = "Index of Date")
# set the dataset to training and testing data with a ratio of 2/3 and 1/3
# LSTM is sensitive to data so we need to seize data to [0,1]
max_value <- max(dataframe$Value)
min_value <- min(dataframe$Value)
spread <- max_value - min_value

create_dataset <- function(dataset,look_back = look_back){
  l <- length(dataset)
...
dataset <- (dataframe$Value - min_value) / spread
results <- data.frame(lookback=0, train_err=0, test_err = 0)
set.seed(7)
look_backs <- seq(1,50,2)
...
for(look_back in look_backs){
    ...
    ggsave(p, filename = paste("./BITCOIN/BITCOIN-lookback-",as.character(look_back),".png"), width = 600, height = 400, units = "mm")
    rm(model)
}
