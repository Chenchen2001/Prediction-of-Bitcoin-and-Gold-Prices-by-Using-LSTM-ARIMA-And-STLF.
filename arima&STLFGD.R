...
train_size <- as.integer(length(data$USD) * 0.67)
test_size <- length(data$USD) - train_size

train <- ts(data[1:train_size,]$USD, frequency = 365)
test <- data[(train_size + 1):length(data$USD),]$USD
